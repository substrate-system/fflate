...
# With STALL_LIMIT=4 the loop runs at stall 0,1,2,3 before bailing, and this
# three-entry ladder clamps, so those map to medium, high, max, max. Progress
# resets STALLED_COUNT=0, which drops effort back to the baseline
# automatically -- no separate "reset to medium" logic needed.
...
# The single "nothing headless left" exit. count_pending() excludes manual:true
# stories, so 0 pending does NOT mean the PRD is finished -- claiming a clean
# sweep while a manual story is still open would be a lie. Both callers (the
# step-1 "no task selected" check and the step-5 COMPLETE check) funnel through
# here so the two paths cannot report different things about the same state.
report_done_and_exit() {
    local pending_manual
...
# Did the session actually SIGNAL completion? Grepping the raw jsonl does not
# work: the stream carries the model's thinking blocks and tool inputs, and the
# STOP CONDITION section of PROMPT.md -- which this script feeds in verbatim --
# quotes the token itself, so a plain grep matches any session that merely
# reasons about when to stop. Look only at text the assistant actually emitted,
# and only at its LAST message, since PROMPT.md says to print the token and then
# do no further work. A false negative here is harmless: the next iteration's
# step-1 check reaches the same conclusion from the PRD and exits cleanly.
saw_complete() {
    jq -rs '
        [ .[]
    ...
    ' "$1" 2>/dev/null | grep -q '<promise>COMPLETE</promise>'
}
...
while [ "$ITERATION" -lt "$MAX_ITERATIONS" ]; do
    ...
    # 2. Stall-driven friction relief: wipe the build output.
    #    dist/ is the only generated state a session can get stuck against.
    #    It is gitignored, and the tests import it directly rather than
    #    importing src/ (test/3-node-min.ts, test/browser/index.ts,
    #    test/browser/min.ts). A session that edits src/ and then tests
    #    without rebuilding is measuring the OLD code, so its fix "does
    #    nothing" and it flails -- exactly the shape of a stall. Deleting
    #    dist/ makes the staleness loud instead of silent: the next
    #    test:browser fails outright until `npm run build` runs, which
    #    `npm test` does first anyway.
    if [ "$STALLED_COUNT" -eq 2 ] && [ -d "dist" ]; then
        log_warn "Friction detected -- clearing dist/ to force a clean rebuild ..."
        rm -rf dist
    fi
    ...
    # 5. Detect completion -- but ONLY trust it if the PRD agrees. saw_complete()
    #    already ignores thinking blocks and mid-session chatter; this second
    #    gate catches a session that signals done while work is demonstrably
    #    left. report_done_and_exit() then reports any remaining manual stories
    #    rather than claiming a clean sweep.
    PENDING_AFTER="$(count_pending)"
    if saw_complete "$TMP_CAPTURE"; then
        if [ "$PENDING_AFTER" -eq 0 ]; then
            log_success "COMPLETE signalled and no headless tasks pending."
            report_done_and_exit
        else
            log_warn "Saw COMPLETE signal but $PENDING_AFTER task(s) still pending -- ignoring."
        fi
    fi
