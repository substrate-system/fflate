            ...
            p['create' + k.slice(0, 1).toUpperCase() + k.slice(1)] = function (
                ...
                const out = {
                    ...
                    push (v:Uint8Array, f:boolean) {
                        if (!out.ondata) throw new Error('no callback')
                        trueCb = out.ondata
...
const exec = (code:string, files:File[], callback:Callback) => {
    ...
    try {
        // Evaluating the user's snippet is what this module is for; the
        // demo page has no behaviour without it.
